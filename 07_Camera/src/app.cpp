...
void App::run(){
  ...
  renderer.init_vao();
  renderer.init_vbo(square, sizeof(square), GL_STATIC_DRAW, sizeof(float) * 8, square_data);
  renderer.init_ebo(square_indices, sizeof(square), GL_STATIC_DRAW, sizeof(float) * 8); 
  ...
  while(!glfwWindowShouldClose(renderer.m_window.get_window())){
    ...
    renderer.draw_ebo(GL_TRIANGLES, indice_count, GL_UNSIGNED_INT);
